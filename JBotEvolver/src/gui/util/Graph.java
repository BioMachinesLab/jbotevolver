...
public class Graph extends JPanel {
	...
	private static final Color[] COLORS = { new Color(204, 37, 41), // Red
			...
			if (divider > 0) {
				for (int i = 0; i <= divs; i++) {
					...
					if (min >= 0 || i != 0) {
						...
						double toAdd = !listOfData.isEmpty() && listOfData.get(0).size() > showLast
								? listOfData.get(0).size() - showLast : 0;
	...
	private void drawLabels(Graphics2D g2) {
		...
		String s = yLabel;
		int stringLenght = g2.getFontMetrics().stringWidth(s);
		float sy = ((h - padTop) / 2) + padTop - stringLenght;
		float sw = (float) font.getStringBounds(s, frc).getHeight();

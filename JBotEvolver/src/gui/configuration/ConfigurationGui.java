...
public class ConfigurationGui extends Gui {
	...
	private String[] keys = { "output", "robots", "controllers", "population", "environment", "executor", "evolution",
	...
	private void fillBlackList() {
		 ...
		 * blackList.add("CompassSensor"); blackList.add("ConeTypeSensor");
		 * blackList.add("DoubleParameterSensor");
		 * blackList.add("EpuckIRSensor"); blackList.add("EpuckLightSensor");
		 * blackList.add("LightTypeSensor"); blackList.add("NearRobotSensor");
		 * blackList.add("NestSensor");
		 * blackList.add("PerimeterSimpleLightTypeSensor");
		 * blackList.add("PerimeterSimpleRobotColorSensor");
		 * blackList.add("PositionSensor"); blackList.add("PreySensor");
		 * blackList.add("RobotColorSensor");
		 * blackList.add("RobotRGBColorSensor"); blackList.add("RobotSensor");
		 * blackList.add("Sensor"); blackList.add("SimpleMouseSensor");
		 * blackList.add("SimpleNestSensorPerimeter");
		 * blackList.add("SimplePreySensorPerimeter");
		 * blackList.add("SimpleTeamNestSensor");
		 * blackList.add("WallButtonSensor"); blackList.add("WallSensor");
		 * blackList.add("WaypointSensor"); blackList.add("XRayPreySensor");
		 * blackList.add("OpenDoorActuator");
		 * blackList.add("RobotColorActuator");
		 * blackList.add("RobotRGBColorActuator");
		 * blackList.add("StopActuator"); blackList.add("Epuck");
		 ...
		 * blackList.add("DestructiveBehavior"); blackList.add("DummyBehavior");
		 * blackList.add("MoveForwardBehavior");
		 * blackList.add("OpenDoorBehavior"); blackList.add("TurnLeftBehavior");
	...
	private Component initSelectionPanel() {
		...
		subPanelNorth2.add(createJComboBoxPanel(new Class<?>[] { Robot.class, Sensor.class, Actuator.class })); // 2.
																												// Robots
		subNorth.add(subPanelNorth2);

		JPanel subPanelNorth3 = new JPanel(new GridLayout(1, 1));
		subPanelNorth3.add(createJComboBoxPanel(new Class<?>[] { Controller.class, NeuralNetwork.class })); // 3.
																											// Controllers
		subNorth.add(subPanelNorth3, BorderLayout.SOUTH);

		north.add(subNorth, BorderLayout.NORTH);
		north.add(createJComboBoxPanel(new Class<?>[] { Population.class, Environment.class, TaskExecutor.class,
				Evolution.class, EvaluationFunction.class }));
	...
	private Component initConfigurationPanel() {
		...
		JScrollPane scroll = new JScrollPane(splitLayout, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	private Component initOutputWrapperPanel() {
		...
		randomSeedTextField.getDocument()
				.addDocumentListener(new TextFieldListener(randomSeedTextField, "random-seed"));
	...
	private Component createJComboBoxPanel(Class<?>[] classNames) {
		...
		LinkedList<String> nameExceptions = new LinkedList<String>(
				Arrays.asList(new String[] { "controller", "robot", "actuator", "sensor" }));
		...
		testArgumentsFileButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {
				...
				try {
					...
					TaskExecutor taskExecutor = TaskExecutor.getTaskExecutor(jBotEvolver,
	...
	private Renderer setupRenderer() {
		...
		try {
			...
			Simulator simulator = new Simulator(new Random().nextLong(), arguments);
			...
			e.printStackTrace();
			// Gotta catch 'em all! Silence the exceptions that might occur
	...
	private void specialAddToConfigFile(String key, Arguments arguments, String className) {
		if (key.equals("robots")) {
			...
			for (int i = 0; i < actuatorsArguments.getArguments().size(); i++) {
				currentClassName = new Arguments(actuatorsArguments.getValues().get(i))
						.getArgumentAsString("classname");
				currentClassName = currentClassName.split("\\.")[currentClassName.split("\\.").length - 1];
	...
	private void populateOptionsPanel(ArrayList<Class<?>> classesList, String className,
			ArrayList<AutomatorOptionsAttribute> optAttributes) throws ClassNotFoundException {
		cleanOptionsPanel();
		...
		int height = annotations.size() + 1 >= OPTIONS_GRID_LAYOUT_SIZE ? annotations.size() + 1
				: OPTIONS_GRID_LAYOUT_SIZE;
	...
	private JTextField createOptionPanelTextField(AutomatorOptionsAttribute attribute, String defaultValue,
			boolean editMode) {
	...
	private JComboBox<String> createOptionPanelComboBox(ArgumentsAnnotation annotation,
			AutomatorOptionsAttribute attribute) {
	...
	private JComboBox<String> editOptionPanelComboBox(ArgumentsAnnotation annotation,
			AutomatorOptionsAttribute attribute, String value) {
		JComboBox<String> combo;
		combo = new JComboBox<String>();
	...
	private Component createCheckBox(Component component, AutomatorOptionsAttribute attribute,
			boolean editedAttribute) {
	...
	private void editOptionPanelAttribute() {
		try {
			...
			if (attributeID != null) {
				...
				optionsAttributes.clear();
				populateOptionsPanel(ClassLoadHelper.findClassesContainingName(c),
						args.getArgumentAsString("classname"), optionsAttribute);

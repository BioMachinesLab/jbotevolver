...
public abstract class Population implements Serializable {
	...
	public Population(Arguments arguments) {
		...
		if (arguments.getArgumentIsDefined("increasestepsperrun")) {
			setIncreasingStepPerFitnessSample(arguments.getArgumentAsInt("startsteps"),
					arguments.getArgumentAsInt("endsteps"), arguments.getArgumentAsDouble("stepspergeneration"));
		}
		...
		if (arguments.getArgumentIsDefined("increasesamplesperindividual")) {
			setIncreasingSamplesPerFitnessSample(arguments.getArgumentAsInt("startsamples"),
					arguments.getArgumentAsInt("endsamples"), arguments.getArgumentAsDouble("samplespergeneration"));
		}
	...
	/**
	 * Get the number of chromosomes that have already been evaluated in the
	 * current generation.
	 * 
	...
	/**
	 * Gets the next chromosome to evaluate. Returns the next chromosome if
	 * there are one or more chromosomes to evaluate at the time of the call. In
	 * case all the chromosomes have been/are being evaluated, null is returned.
	 * <p>
	...
	/**
	 * Set the fitness for a chromosome obtained by a call to
	 * {@link #getNextChromosomeToEvaluate()}.
	 * 
	 * @param chromosome
	 * @param fitness
	 */
	...
	/**
	 * Set the fitness for a chromosome in position pos obtained by a call to
	 * {@link #getNextChromosomeToEvaluate()}.
	 * 
	 * @param chromosome
	 * @param fitness
	...
	/**
	 * Creates the next generation. Once all of the chromosomes in a generation
	 * has been evaluated, call this method to create the next generation. This
	 * method should apply generation operators such as selection mutation and
	 * cross-over etc.
	 */
	 ...
	 * 
	 * @param seed
	 *            the random seed for this generation.
	 */
	...
	/**
	 * Get the chromosome that obtained the highest fitness in the current
	 * generation.
	 * 
	 * @return chromosome that obtained the highest fitness in the current
	 *         generation.
	 */
	public abstract Chromosome getBestChromosome();

	/**
	 * Get the top chromosomes that obtained the highest fitness in the current
	 * generation.
	 * 
	 * @param number
	 *            of chromossomes to be returned
	 * 
	 * @return chromosome[] that obtained the highest fitness in the current
	 *         generation.
	 */
	 ...
	 * 
	 * @param generations
	 *            the number generations to evolve.
	 */
	...
	public int getNumberOfSamplesPerChromosome() {
		if (enableIncreasingSamplesPerChromosome) {
			...
			samplesPerChromosome += getNumberOfCurrentGeneration() * increaseAdditionalSamplesPerGeneration;
	 ...
	 * 
	 * This will cancel any previously set increasing number of samples (see
	 * {@link #enableIncreasingSamplesPerChromosome}).
	 * 
	 * @param argumentAsInt
	 *            the number of fitness samples per chromosome.
	 */
	...
	/**
	 * Set the number of fitness samples to increase throughout an evolution.
	 * This allows for chromosomes to be sampled quickly (fewer samples) in the
	 * beginning of an evolutionary run and more accurately later in an
	 * evolutionary run (more samples).
	 * 
	 * @param startSamples
	 *            the number of samples for the first generation.
	 * @param maxSamples
	 *            the max number of samples -- the number of samples will not be
	 *            increased beyond this number.
	 * @param additionalSamplesPerGeneration
	 *            the number of additional samples per generation (can be a
	 *            fraction).
	 * 
	 */
	public void setIncreasingSamplesPerFitnessSample(int startSamples, int maxSamples,
			double additionalSamplesPerGeneration) {
	...
	/**
	 * Set the number of simulation steps per fitness sample to increase
	 * throughout the evolution. This allows for chromosomes to be sampled
	 * quickly in the beginning of an evolutionary run and more accurately later
	 * in an evolutionary run.
	 * 
	 * @param startStepsPerRun
	 *            the number of steps per fitness sample for the first
	 *            generation.
	 * @param maxSteps
	 *            the max number of steps -- the number of steps will not be
	 *            increased beyond this number.
	 * @param additionalStepsPerRunPerGeneration
	 *            the number of additional steps per generation (can be a
	 *            fraction).
	 * 
	 */
	public void setIncreasingStepPerFitnessSample(int startStepsPerRun, int maxStepsPerRun,
	...
	/**
	 * Enable fitness threshold-based stop of an evolution. If the best
	 * chromosome scores a fitness above the fitness threshold for a certain
	 * number of consecutive generations, the evolution is stopped. This
	 * function is useful for incremental evolutions where the task is made
	 * increasingly more difficult in order to bootstrap evolution.
	 *
	 * @param fitnessThreshold
	 *            the fitness threshold value
	 * @param numberOfGenerationsAboveFitnessThresholdRequired
	 *            the number of generations the fitness has to be above the
	 *            threshold before evolution is terminated.
	 */
	public void enableFitnessThreshold(double fitnessThreshold, int numberOfGenerationsAboveFitnessThresholdRequired) {
	...
	/**
	 * Check if the fitness threshold has been reached. This method should be
	 * called by a specialization of this class when all the chromosomes in a
	 * generation have been evaluated. If fitness-based threshold stops have not
	 * been enabled, it will return false. Otherwise, it will perform the
	 * necessary checks and return true if the fitness threshold has been
	 * reached.
	 * 
	 * @param bestFitnessOfCurrentGeneration
	...
	private static Population loadPopulationFromFile(Arguments args) throws Exception {
		...
		String parentFolder = args.getArgumentAsString("parentfolder") != null
				? args.getArgumentAsString("parentfolder") : "";

		File populationFile = new File(parentFolder, "populations" + File.separatorChar + f.getName());
		if (!populationFile.exists()) {
			File folder = new File(parentFolder);
			populationFile = new File(folder.getParentFile(), "populations" + File.separatorChar + f.getName());
		}

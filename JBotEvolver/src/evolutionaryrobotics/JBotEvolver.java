		...
		if (evalArgs != null) {
			...
			if (!evalArgs.getArgumentIsDefined("multieval"))
				...
				for (int i = 0; i < evals.length; i++) {
					evals[i] = EvaluationFunction.getEvaluationFunction(
							new Arguments(multiObjectives.getArgumentAsString(multiObjectives.getArgumentAt(i))));
				}
			}
		} else
		...
		if (arguments != null) {
			if (arguments.get("--output") != null) {
				if (arguments.get("--bestcontrollerweights") != null
					...
					if (new File(arguments.get("--output").getCompleteArgumentString()).exists()) {
						absolutePath = new File(arguments.get("--output").getCompleteArgumentString()).toString();
					} else {

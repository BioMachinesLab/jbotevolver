...
class JMapViewerImplementation extends JPanel {
	private static final long serialVersionUID = -3098978188474820429L;
	...
	public JMapViewerImplementation(String name) {
		this(name, false);
	}

	public JMapViewerImplementation(String name, boolean treeVisible) {
		super();
		...
		tree.addNodeListener(new MouseAdapter() {
			...
			private void maybeShowPopup(MouseEvent e) {
				if (e.isPopupTrigger()) {
					...
					if (layer != null)
						JMapViewerImplementation.this.createPopupMenu(layer).show(e.getComponent(), e.getX(), e.getY());
				}
			}
		});
	}
	...
	public JMapViewerImplementation addLayer(Layer layer) {
		tree.addLayer(layer);
		layers.add(layer);
		return this;
	}

	public JMapViewerImplementation addLayer(MapObject element) {

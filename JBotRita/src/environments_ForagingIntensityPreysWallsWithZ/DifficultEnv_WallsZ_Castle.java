...
public class DifficultEnv_WallsZ_Castle extends ForagingIntensityPreysEnvironment_Z {
	...
	private static final double MAX_Y_LIMIT_FOR_PREY_CENTER_DIFFICULTENV = -0.8;
	private static final double MIN_Y_LIMIT_FOR_PREY_CENTER_DIFFICULTENV = -1.7;
	...
	private static final double MIN_Y_LIMIT_FOR_PREY_LEFT_DIFFICULTENV = -1.7;
	
	private boolean firstPositionOfPreyWasAdded =false;
	
	
	
	public DifficultEnv_WallsZ_Castle(Simulator simulator, Arguments arguments) {
	...
	public void init(Environment env){   		
		...
		env.addStaticObject(new Wall(simulator, 1.9, 0, 0.1, 3.8)); // vertical east in the forbidden area
		env.addStaticObject(new Wall(simulator, -1.8, 0, 0.1, 3.8)); // vertical west in the forbidden area
		...
		env.addStaticObject(new Wall(simulator, -0.2, -1.9, 1.3, 0.1));  // horizontal south in the forbidden area

...
public class MyTraceRender extends Renderer {
	...
	public MyTraceRender(Arguments args) {
		super(args);
	...
	public void outputFrame(int frameNumber) {		
		...
		if (terminal == JPG) {
			try {
				ImageIO.write(image, "png", new File(filenameBase + frameNumber + ".pdf"));
			} catch (IOException e) {
				System.out.println("Could not save file: " + filenameBase + frameNumber + ".pdf, reason: " + e.getMessage());
				e.printStackTrace();
			}
		}

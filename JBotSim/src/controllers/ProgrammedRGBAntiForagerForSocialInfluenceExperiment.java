...
public class ProgrammedRGBAntiForagerForSocialInfluenceExperiment extends
	...
	//
	// private Random rand = SimRandom.getInstance();
	//
	...
	//
	// private enum Action {
	// NA, TURN_LEFT, TURN_RIGHT, FORWORD, GRAB, RELEASE, TURN_RIGHT_AVOID,
	// BACKWARD, LOST, FULL_FORWORD
	// };
	//
	...
	//
	// public ProgrammedAntiForagerForSocialInfluenceExperiment(Robot robot,
	// Arguments arguments) {
	// super(robot);
	// super(robot);
	// double speed = 0;
	...
	// // }
	//
	// nearPreyDistance = (foodSensors.getRange() - robot.getRadius() -
	// ((RoundForageEnvironment) Environment
	// .getInstance()).getPrey().get(0).getDiameter())
	...
	public ProgrammedRGBAntiForagerForSocialInfluenceExperiment(Simulator simulator,Robot robot,
			Arguments arguments) {
		super(simulator,robot, arguments);
		double range = nestSensors.getRange();
		onLimitDistance = .95 //1.02
				* (range - ((RoundForageEnvironment) simulator.getEnvironment())
						.getForageRadius()) / range;
		grabLimit = (range - .9 * ((RoundForageEnvironment) simulator.getEnvironment()).getForageRadius()) / range;
		RED_COLOR_LEVEL = 1.0;
	}
...
//		double right_sensor_measure;
		...
		if (isLoaded) {
			...
			// GET FOOD
			if (maxNestReading < grabLimit && maxNestReading > 0) {
		...
		} else if (allFrontSensorsZero(foodSensors) && allowedForagingArea) {
			newAction = Action.FORWORD;

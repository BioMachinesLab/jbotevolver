...
public class OldProgrammedForager extends Controller {
	...
	private static final int NUM_AVOID_STEPS = (int) (1 / DELTA_SPEED) / 4;
	private static final int NUM_AVOID_STEPS_TURN = 2*NUM_AVOID_STEPS / 3;
	...
	public OldProgrammedForager(Simulator simulator, Robot robot,
		...
		inNestSensor = (InNestSensor) robot.getSensorWithId(6);
	...
	private Action chooseAction() {
		...
		if (isLoaded && inNestSensor.isInNest()
				&& front_sensor_measure > left_sensor_measure
				&& front_sensor_measure > right_sensor_measure) {// near(nestSensors,
																	// onNest))
																	// { // ||
			...
			newAction = Action.GRAB;
															...
															// robotRedSensor.getSensorReading(0)
															// >
															// MIN_COLIDE_DISTANCE
				&& (front_sensor_measure <= robotRedSensor.getSensorReading(0)
//						|| left_sensor_measure <= robotRedSensor.getSensorReading(1) 
//						|| right_sensor_measure <= robotRedSensor.getSensorReading(7)
						)) {
	...
	private boolean lost() {
		multFactor *= .995;
		motors.setWheelSpeed(moveForwardSpeed, moveForwardSpeed * Math.min(.98, 1 - multFactor));
...
//		double factorAvoid = 1;
//		if (tray.getSensorReading(0) == 0 && front_sensor_measure < robotBlueSensor.getSensorReading(0))
//			factorAvoid = 5;
//				.5*(robotBlueSensor.getSensorReading(1) + robotBlueSensor.getSensorReading(2) + robotBlueSensor.getSensorReading(3)
//					- robotBlueSensor.getSensorReading(7) - robotBlueSensor.getSensorReading(6) - robotBlueSensor.getSensorReading(5));
		if (right_sensor_measure < left_sensor_measure) {
			motors.setWheelSpeed(
					moveForwardSpeed
							/ (1 + 5 * (left_sensor_measure - right_sensor_measure) ),
					moveForwardSpeed);
		} else {
			motors.setWheelSpeed(
					moveForwardSpeed,
					moveForwardSpeed
							/ (1 + 5 * (right_sensor_measure - left_sensor_measure)));
		}
		return true;
	}
	...
	private boolean moveForward() {
		factor = INIT_FACTOR_VALUE;
//		double factorAvoid = 1;
		// if(tray.getSensorReading(0) == 0)
		// factorAvoid = robotBlueSensor.getSensorReading(1) -
		// robotBlueSensor.getSensorReading(7);
		//

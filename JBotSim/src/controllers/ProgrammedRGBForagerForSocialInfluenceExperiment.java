...
public class ProgrammedRGBForagerForSocialInfluenceExperiment extends
		Controller {

	// private static final int TURNSPEED = 300;

	protected static final double MAXSPEED = Robot.MAXIMUMSPEED;
	...
	protected double RED_COLOR_LEVEL = 0;
	...
	protected RobotRGBColorActuator topLight;
	protected RobotRGBColorSensor robotRGBColorSensor;
	...
	public ProgrammedRGBForagerForSocialInfluenceExperiment(
		...
		robotRGBColorSensor = (RobotRGBColorSensor) (robot.getSensorWithId(4));
		...
		// }else if (name.equalsIgnoreCase("robotcolor")) {
		topLight = (RobotRGBColorActuator) robot.getActuatorWithId(3);
		// }
		// }
	...
	protected void updateTopLight(boolean isLoaded) {
		topLight.setGreen(0);
		if (isLoaded) {
			topLight.setGreen(1);
		}
	...
	protected Action chooseAction() {
		...
		if (isLoaded && inNestSensor.isInNest()) { // near(nestSensors, onNest))
			...
			// && front_sensor_measure <= robotRedSensor.getSensorReading(0)) {
				...
				&& front_sensor_measure <= robotRGBColorSensor.getDistance(0)
				&& robotRGBColorSensor.getGreen(0) > 0.5) {
	...
	protected boolean detourRight(double speed) {
		motors.setWheelSpeed(speed, speed * .3);
		return true;
	}
	...
	protected boolean moveForward() {
		...
		if (sensor.getSensorReading(0) < robotRGBColorSensor.getDistance(0)
				&& robotRGBColorSensor.getGreen(0) > .5) {
			crashFactor -= robotRGBColorSensor.getDistance(0);
		}

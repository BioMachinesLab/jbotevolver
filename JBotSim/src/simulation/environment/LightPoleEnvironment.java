...
public class LightPoleEnvironment extends Environment {
	 ...
	 * 
	 * @param list
	 *            , the positions list of the poles
	 * @param radius
	 * @return list of poles
	 */
	private ArrayList<LightPole> createPoles(ArrayList<Vector2d> list,
			double radius) {
		LightPole light;
		ArrayList<LightPole> polesToAdd = new ArrayList<LightPole>();
		for (int i = 0; i < list.size(); i++) {
			light = new LightPole(simulator, "pole " + Integer.toString(i),
					list.get(i).x, list.get(i).y, radius);
	...
	private ArrayList<LightPole> createMultipleEnvironments(double radius) {
		...
		for (int i = 0; i < positions.size(); i++) {
			light = new LightPole(simulator, "pole " + Integer.toString(i),
					positions.get(i).x, positions.get(i).y, radius);
			polesToAdd.add(light);
	...
	private ArrayList<Vector2d> getPositions(int numberOfPoles) {
		...
		if (numberOfPoles == 1) {
			...
			for (int i = 1; i <= numberOfPoles; i++) {
				list.add(new Vector2d(lightPoleDistance
						* Math.cos(2 * (Math.PI) * i / numberOfPoles),
						lightPoleDistance
								* Math.sin(2 * (Math.PI) * i / numberOfPoles)));
	...
	private ArrayList<Vector2d> getRandomPositions(int numberOfPoles) {
		...
		do {
			...
			for (int i = 0; i < numberOfPoles; i++) {
				...
				list.add(new Vector2d(this.getWidth()
						* (simulator.getRandom().nextDouble() - 0.5), this
						.getHeight()
						* (simulator.getRandom().nextDouble() - 0.5)));
